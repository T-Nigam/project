...
static void test_color_bars(void) {
  ...
  uint8_t colors[8] = {COLOR_WHITE,   COLOR_YELLOW, COLOR_CYAN, COLOR_GREEN,
...
static void test_moving_ball(void) {
  ...
  for (int frame = 0; frame < 300; frame++) { // run for 300 frames then stop
    ...
    // Bounce off walls
    if (ball_x - ball_r < 1 || ball_x + ball_r > SCREEN_WIDTH - 1)
      ball_vx = -ball_vx;
    if (ball_y - ball_r < 1 || ball_y + ball_r > SCREEN_HEIGHT - 1)
      ball_vy = -ball_vy;
  }
}
