...
// Pack three TMDS control symbols (one per color channel) into one 32-bit word.
...
static uint32_t vactive_line[] = {HSTX_CMD_RAW_REPEAT | DVI_H_FRONT_PORCH,
                                  SYNC_V1_H1,
                                  HSTX_CMD_NOP,
                                  HSTX_CMD_RAW_REPEAT | DVI_H_SYNC_WIDTH,
                                  SYNC_V1_H0,
...
static void __scratch_x("dvi") dma_irq_handler(void) {
  ...
  if (v_scanline >= DVI_V_FRONT_PORCH &&
    ...
    ch->transfer_count = count_of(vblank_line_vsync_on);
             ...
             DVI_V_FRONT_PORCH + DVI_V_SYNC_WIDTH + DVI_V_BACK_PORCH) {
    ...
    ch->read_addr =
        (uintptr_t)(frame_display_ptr + active_line * DVI_H_ACTIVE_PIXELS);
...
#if DVI_DETECT_ACTIVE_LOW
  gpio_pull_up(DOCK_DETECT_PIN);
#else
...
void dvi_set_display_buffer(uint8_t *buf) { frame_display_ptr = buf; }
...
bool dvi_screen_connected_raw(void) { return gpio_get(DOCK_DETECT_PIN); }

bool dvi_screen_connected(void) {
#if DVI_DETECT_ACTIVE_LOW
  return !dvi_screen_connected_raw();
#else
  return dvi_screen_connected_raw();
#endif
}
